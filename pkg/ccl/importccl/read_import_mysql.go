...
func mysqlColToCockroach(
	...
	case mysqltypes.Set:
		return nil, errors.UnimplementedWithIssueHint(32560,
	...
	case mysqltypes.Geometry:
		return nil, errors.UnimplementedWithIssuef(32559,
			"cannot import GEOMETRY columns at this time")
	case mysqltypes.Bit:
		return nil, errors.UnimplementedWithIssueHint(32561,
			"cannot improt BIT columns at this time",
			"try converting the column to a 64-bit integer before import")
	default:
		return nil, errors.Unimplementedf(fmt.Sprintf("import.mysqlcoltype.%s", typ), "unsupported mysql type %q", col.Type)
	}
	...
	if col.Default != nil && !bytes.EqualFold(col.Default.Val, []byte("null")) {
		...
		if col.Default.Type == mysql.StrVal {
			...
			if err != nil {
				return nil, errors.Unimplementedf("import.mysql.default", "unsupported default expression %q for column %q: %v", exprString, name, err)
			}
			def.DefaultExpr.Expr = expr
		}
	}
