...
func (expr *IndirectionExpr) TypeCheck(ctx *SemaContext, desired *types.T) (TypedExpr, error) {
	for i, t := range expr.Indirection {
		if t.Slice {
			return nil, errors.UnimplementedWithIssuef(32551, "ARRAY slicing in %s", expr)
		}
		if i > 0 {
			return nil, errors.UnimplementedWithIssueDetailf(32552, "ind", "multidimensional indexing: %s", expr)
		}
...
func (sc *SemaContext) checkFunctionUsage(expr *FuncExpr, def *FunctionDefinition) error {
	if def.UnsupportedWithIssue != 0 {
		...
		if def.UnsupportedWithIssue < 0 {
			return errors.Unimplemented(def.Name+"()", msg)
		}
		return errors.UnimplementedWithIssueDetail(def.UnsupportedWithIssue, def.Name, msg)
	}
	if def.Private {
