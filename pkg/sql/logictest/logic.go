...
func (t *logicTest) execStatement(stmt logicStatement) (bool, error) {
	...
	statementModifiers := []func(string)string {
		t.assignRandomFamily,
		t.alterPrimaryKey,
	}
	execSQL := stmt.sql
	for _, f := range statementModifiers {
		execSQL = f(execSQL)
	}
...
// Create another index that is the same as the primary key, and issue
// an alter primary key statement.
func (t *logicTest) alterPrimaryKey(sql string) string {
	...
	for _, stmt := range stmts {
		...
		// 3 cases:
		// 1. A single column is the primary key.
		// 2. Multiple columns are set up as the primary key.
		// 3. The create table doesn't contain a primary key -- an autogenerated one is created.
		// TODO (rohany): I don't think we can support case 3 right now (requires some index rewrites).
		var columnsInPK tree.IndexElemList
		for _, def := range ast.Defs {
			// Ensure that we don't end up with extra pk columns.
			if len(columnsInPK) != 0 {
				break
			}
			switch t := def.(type) {
			case *tree.ColumnTableDef:
				if t.PrimaryKey {
					columnsInPK = tree.IndexElemList{tree.IndexElem{Column:t.Name}}
				}
			case *tree.UniqueConstraintTableDef:
				if t.PrimaryKey {
					for _, elem := range t.IndexTableDef.Columns {
						columnsInPK = append(columnsInPK, elem)
					}
				}
			default:
				continue
		...
		// If we didn't find any explicit primary key declaration, don't do anything for now.
		if len(columnsInPK) == 0 {
			continue
		}

		// Create a secondary index on the table that indexes the pk columns, and stores the
		// rest of the columns in the table.
		idx := &tree.CreateIndex{
			Name: "generated_newprimaryidx",
			Table: ast.Table,
			Unique: true,
			Columns: columnsInPK,
			Storing: make(tree.NameList, 0),
		}
		for _, def := range ast.Defs {
			...
			// See if we index this column already.
			inIndex := false
			for _, idxElem := range columnsInPK {
				if idxElem.Column == colDef.Name {
					inIndex = true
					break
				}
			}
			if !inIndex {
				idx.Storing = append(idx.Storing, colDef.Name)
			}
		}
		idxCreationString := idx.String()
		sb.WriteString(idxCreationString)
		sb.WriteString(";")
		t.outf("Added secondary index declaration: %s;", idxCreationString)
		// Issue an alter primary key statement now.
		alterPk := &tree.AlterTable{
			Table: ast.Table.ToUnresolvedObjectName(),
			Cmds: tree.AlterTableCmds{
				&tree.AlterTableAlterPrimaryKey{
					TableIndex:tree.TableIndexName{
						Table:ast.Table,
		...
		alterPkString := alterPk.String()
		sb.WriteString(alterPkString)
		sb.WriteString(";")
		t.outf("Generated alter primary key statement: %s;", alterPkString)
	}
